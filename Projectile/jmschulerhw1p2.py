# -*- coding: utf-8 -*-
"""
Created on Thu Sep 06 14:04:54 2018
...
for i in range(1, stepsize+1):
        xp[i]=xp[i-1] + vpx[i-1]*dt - (0.5)*dragpx[i-1]*dt**2     
        yp[i]=yp[i-1] + vpy[i-1]*dt - (0.5)*(g + dragpy[i-1])*dt**2
        if dragpy[i-1] < 0:
            dragpy[i-1] = -dragpy[i-1]
        else: dragpy[i-1] = dragpy[i-1]
        vpy[i] = vpy[i-1] - g*dt - dragpy[i-1]*dt
        vpx[i] = vpx[i-1] - dragpx[i-1]*dt
        if yp[i] <=0:
        ...
        if yp[i-2] < yp[i-1] and yp[i] < yp[i-1]:
            print "Maximum Height Achieved:" ,yp[i-1], "m"
